...
contract SampleSale is OwnableUpgradeable, PausableUpgradeable {
  // zNS Non-registrar, non-domain NFT Contract
  IZNFTUpgradeable public zNFTContract;
  ...
  function initialize(
    ...
    uint256 price_,
    IZNFTUpgradeable zNFTContract_,
  ...
  function setZNFT(IZNFTUpgradeable zNFTContract_) external onlyOwner {
    require(zNFTContract != zNFTContract_, "Same registrar");
    zNFTContract = zNFTContract_;
  }
  ...
  function releaseDomain() external onlyOwner {
    zNFTContract.transferFrom(address(this), owner(), parentDomainId);
  }
  ...
  function adminMint(uint256 amount) external onlyOwner {
    zNFTContract.mint(msg.sender, amount);
  }

  function _mintDomains(uint256 numPurchased) internal {
    // Must register this contract address as a `controller`
    // on the zNFTContract to mint
    zNFTContract.mint(msg.sender, numPurchased);
  }
