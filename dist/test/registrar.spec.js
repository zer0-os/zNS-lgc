...
describe("Registrar", () => {
    ...
    let user2;
    let user3;
    const rootDomainHash = hardhat_1.ethers.constants.HashZero;
    const rootDomainId = ethers_1.BigNumber.from(0);
    const deployRegistry = async (creator) => {
        registryFactory = new typechain_1.Registrar__factory(creator);
        registry = (await hardhat_1.upgrades.deployProxy(registryFactory, [], {
    ...
    describe("root domain", () => {
        ...
        it("creates the root domain belonging to the creator", async () => {
            ...
            expect(domainOwner).to.be.eq(creator.address, "The owner is not the creator.");
        });
        it("tracks the root domain controller as null", async () => {
            expect(await registry.domainController(rootDomainId)).to.eq(hardhat_1.ethers.constants.AddressZero);
        });
    });
    describe("ownership", () => {
    ...
    describe("transferring domains", () => {
        ...
        it("prevents a user who does not own a domain from transferring it", async () => {
            ...
            const tx = registryAsUser1["safeTransferFrom(address,address,uint256)"](creator.address, user1.address, rootDomainId);
            await expect(tx).to.be.revertedWith("ERC721: transfer caller is not owner nor approved");
        });
        it("allows for a domain to be transferred with 'safeTransferFrom'", async () => {
            await registry["safeTransferFrom(address,address,uint256)"](creator.address, user1.address, rootDomainId);
            const domainOwner = await registry.ownerOf(rootDomainId);
    ...
    describe("registering domains", () => {
        ...
        it("prevents non controllers from registering domains", async () => {
            await expect(registry.registerDomain(rootDomainId, "myDomain", user2.address, "", 0, false)).to.be.reverted;
        });
        it("returns false if a domain doesn't exist", async () => {
            await registry.addController(user1.address);
            const domainName = "myDomain";
            const domainNameHash = helpers_1.hashDomainName(domainName);
            const expectedDomainHash = helpers_1.calculateDomainHash(rootDomainHash, domainNameHash);
        ...
        it("allows controllers to register domains", async () => {
            ...
            const domainName = "myDomain";
            const domainNameHash = helpers_1.hashDomainName(domainName);
            await registryAsUser1.registerDomain(rootDomainId, domainName, user2.address, "", 0, false);
            const expectedDomainHash = helpers_1.calculateDomainHash(rootDomainHash, domainNameHash);
            const doesDomainExist = await registry.domainExists(expectedDomainHash);
        ...
        it("emits a DomainCreated event when a domain is registered", async () => {
            ...
            const registryAsUser1 = registry.connect(user1);
            const domainName = "myDomain";
            const tx = await registryAsUser1.registerDomain(rootDomainId, domainName, user2.address, "", 0, false);
        ...
        it("emits a DomainCreated event when a domain is registered with the expected params", async () => {
            ...
            expect(tx)
                ...
                .withArgs(expectedDomainHash, domainName, domainNameHash, expectedParentHash, user2.address, user1.address, "", 0);
        });
        it("emits a Transfer event when a domain is registered (ERC721)", async () => {
            ...
            expect(tx)
                ...
                .withArgs(hardhat_1.ethers.constants.AddressZero, user2.address, expectedDomainHash);
        });
        it("properly tracks who the creator of a domain was", async () => {
            await registry.addController(user1.address);
            const registryAsUser1 = registry.connect(user1);
            const domainName = "myDomain";
            await registryAsUser1.registerDomain(rootDomainId, domainName, user3.address, "", 0, false);
            const domainNameHash = helpers_1.hashDomainName(domainName);
            const expectedDomainHash = helpers_1.calculateDomainHash(rootDomainHash, domainNameHash);
            expect(await registryAsUser1.minterOf(expectedDomainHash)).to.eq(user3.address);
        });
        it("properly tracks the controller that created a domain", async () => {
            ...
            const domainNameHash = helpers_1.hashDomainName(domainName);
            const expectedDomainHash = helpers_1.calculateDomainHash(rootDomainHash, domainNameHash);
            expect(await registryAsUser1.domainController(expectedDomainHash)).to.eq(user1.address);
        });
        it("prevents a domain from being registered if it already exists", async () => {
            ...
            const domainName = "myDomain";
            await registryAsUser1.registerDomain(rootDomainId, domainName, user2.address, "", 0, false);
            const tx = registryAsUser1.registerDomain(rootDomainId, domainName, user3.address, "", 0, false);
        ...
        it("prevents a domain from being registered if it's parent doesn't exist", async () => {
            ...
            const parentHash = helpers_1.hashDomainName(parentName);
            const rootHash = helpers_1.calculateDomainHash(rootDomainHash, parentHash);
            const domainName = "myDomain";
            await expect(registryAsUser1.registerDomain(rootHash, domainName, user2.address, "", 0, false)).to.be.revertedWith("Registrar: No parent");
        });
        it("allows a child domain to be registered on an existing domain", async () => {
            await registry.addController(user1.address);
            const registryAsUser1 = registry.connect(user1);
            const parentName = "myRoot";
            await registryAsUser1.registerDomain(rootDomainHash, parentName, user1.address, "", 0, false);
            const parentHash = helpers_1.hashDomainName(parentName);
            const rootHash = helpers_1.calculateDomainHash(rootDomainHash, parentHash);
            const domainName = "myDomain";
            await registryAsUser1.registerDomain(rootHash, domainName, user1.address, "", 0, false);
            const domainNameHash = helpers_1.hashDomainName(domainName);
            const expectedDomainHash = helpers_1.calculateDomainHash(rootHash, domainNameHash);
    ...
    describe("domain metadata", () => {
        let registryAsUser1;
        let testDomainId;
        let currentExpectedMetadataUri;
        before(async () => {
            await deployRegistry(creator);
        });
        before(async () => {
            ...
            const domainName = "myDomain";
            await registry.registerDomain(rootDomainId, domainName, user1.address, "", 0, false);
            const domainNameHash = helpers_1.hashDomainName(domainName);
            const expectedDomainHash = helpers_1.calculateDomainHash(rootDomainHash, domainNameHash);
        ...
        it("emits an event when a domain's metadata is changed", async () => {
            ...
            await expect(registryAsUser1.setDomainMetadataUri(testDomainId, newMetadataUri))
        ...
        it("updates state when metadata is changed", async () => {
            expect(await registryAsUser1.tokenURI(testDomainId)).to.eq(currentExpectedMetadataUri);
        });
        it("prevents a user who is not the owner of a domain from changing metadata", async () => {
            ...
            await expect(registryAsUser2.setDomainMetadataUri(testDomainId, newMetadataUri)).to.be.revertedWith("Registrar: Not owner");
        });
    });
    describe("domain metadata locking", () => {
        ...
        it("prevents unlocking when metadata is not locked", async () => {
            await expect(registryAsUser1.lockDomainMetadata(testDomainId, false)).to.be.revertedWith("Registrar: Not locked");
        });
        it("prevents a non-owner from locking metadata", async () => {
            const registryAsUser2 = registryAsUser1.connect(user2);
            await expect(registryAsUser2.lockDomainMetadata(testDomainId, true)).to.be.revertedWith("Registrar: Not owner");
        });
        ...
        it("updates state of who locked the metadata", async () => {
            expect(await registryAsUser1.domainMetadataLockedBy(testDomainId)).to.eq(user1.address);
        });
        it("prevents locking metadata if it is already locked", async () => {
            await expect(registryAsUser1.lockDomainMetadata(testDomainId, true)).to.be.revertedWith("Registrar: Metadata locked");
        });
        it("prevents users other than the locker from unlocking metadata", async () => {
            const registryAsUser2 = registryAsUser1.connect(user2);
            await expect(registryAsUser2.lockDomainMetadata(testDomainId, false)).to.be.revertedWith("Registrar: Not locker");
        });
        it("prevents metadata from being set if it is locked", async () => {
            await expect(registryAsUser1.setDomainMetadataUri(testDomainId, "https://www.chuckecheese.com/")).to.be.revertedWith("Registrar: Metadata locked");
        });
    ...
    describe("domain royalties", () => {
        ...
        it("emits a RoyaltiesAmountChanged event when a domain's royalty amount changes", async () => {
            ...
            const tx = await registryAsUser1.setDomainRoyaltyAmount(testDomainId, royaltyAmount);
            expect(tx)
        ...
        it("updates state when a domain's royalty amount changes", async () => {
            expect(await registryAsUser1.domainRoyaltyAmount(testDomainId)).to.eq(currentExpectedRoyaltyAmount);
        });
        it("prevents a user who is not the domain owner from setting the royalty amount", async () => {
            const registryAsUser2 = registryAsUser1.connect(user2);
            await expect(registryAsUser2.setDomainRoyaltyAmount(testDomainId, 0)).to.be.revertedWith("Registrar: Not owner");
        });
    });
});
