...
contract StakingController is IZNSController, Initializable {
    ...
    function claimBid(
        ...
        (uint256 id, uint256 parentId) = registry.createDomainSafeController(domain, owner, controller, data);
        Stake storage stake =
    ...
    function safeClaimBid(
        ...
        (uint256 id, uint256 parentId) = registry.safeCreateDomain(
            domain,
            owner,
            controller,
            mintData,
    ...
    function bidForByPath(
        ...
        uint256 amt = bancorNetwork().convertByPath(_path, _amount, _minReturn, payable(address(this)), address(0), 0);
        {
            ...
            bytes32 stakeHash =
                keccak256(abi.encodePacked(staker, id, parentId));
            Stake storage stake = stakes[stakeHash];
