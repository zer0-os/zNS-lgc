...
contract ConverterUpgrader is IConverterUpgrader, ContractRegistryClient {
    IERC20Token private constant ETH_RESERVE_ADDRESS =
     ...
     */
    event ConverterUpgrade(
        address indexed _oldConverter,
        address indexed _newConverter
    );
     ...
     */
    constructor(IContractRegistry _registry, IEtherToken _etherToken)
        public
    ...
    function upgradeOld(IConverter _converter, bytes32 _version) public {
        ...
        bool activate =
            isV28OrHigherConverter(converter) && converter.isActive();
     ...
     */
    function createConverter(IConverter _oldConverter)
        private
        returns (IConverter)
    {
    ...
    function handleTypeSpecificData(
        ...
        if (converterType == 2) {
            ILiquidityPoolV2Converter oldConverter =
                ILiquidityPoolV2Converter(address(_oldConverter));
            ILiquidityPoolV2Converter newConverter =
                ILiquidityPoolV2Converter(address(_newConverter));

            uint16 reserveTokenCount = oldConverter.connectorTokenCount();
            for (uint16 i = 0; i < reserveTokenCount; i++) {
                // copy reserve staked balance
                IERC20Token reserveTokenAddress =
                    oldConverter.connectorTokens(i);
                uint256 balance =
                    oldConverter.reserveStakedBalance(reserveTokenAddress);
                newConverter.setReserveStakedBalance(
                    reserveTokenAddress,
                    balance
                );
            }
            ...
            IERC20Token primaryReserveToken =
                oldConverter.primaryReserveToken();
    ...
    bytes4 private constant IS_V28_OR_HIGHER_FUNC_SELECTOR =
        bytes4(keccak256("isV28OrHigher()"));
    ...
    function isV28OrHigherConverter(IConverter _converter)
        internal
        view
        returns (bool)
    {
        ...
        (bool success, bytes memory returnData) =
            address(_converter).staticcall{gas: 4000}(data);
