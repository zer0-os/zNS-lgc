...
import "../../../converter/types/liquid-token/LiquidTokenConverterProxyable.sol";
 ...
 * has been reached.
 */
contract DynamicLiquidTokenConverterProxyable is LiquidTokenConverterProxyable {
    ...
    event ReserveTokenWeightUpdate(
        uint32 _prevWeight,
        uint32 _newWeight,
        uint256 _percentage,
        uint256 _balance
    ...
    function initialize(
        ...
        LiquidTokenConverterProxyable.initialize(
            _token,
            _registry,
            _maxConversionFee
        );
     ...
     */
    function setMinimumWeight(uint32 _minimumWeight) public ownerOnly inactive {
     ...
     */
    function setStepWeight(uint32 _stepWeight) public ownerOnly inactive {
        ...
        ownerOnly
    {
        ...
        uint256 percentage =
            uint256(PPM_RESOLUTION).sub(newWeight.mul(1e6).div(reserve.weight));
        ...
        uint256 balance =
            reserveBalance(_reserveToken).mul(percentage).div(1e6);

        if (_reserveToken == ETH_RESERVE_ADDRESS) msg.sender.transfer(balance);
        else safeTransfer(_reserveToken, msg.sender, balance);

        lastWeightAdjustmentMarketCap = currentMarketCap;

        syncReserveBalance(_reserveToken);

        emit ReserveTokenWeightUpdate(
            oldWeight,
            weight,
            percentage,
            reserve.balance
    ...
    function withdrawTokens(
        ...
        address _to,
        uint256 _amount
    ) public override protected ownerOnly {
        TokenHolderProxyable.withdrawTokens(_token, _to, _amount);
