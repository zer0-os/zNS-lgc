...
import "../../ConverterBaseProxyable.sol";
 ...
 */
contract LiquidTokenConverterProxyable is ConverterBaseProxyable {
    /**
    ...
    function initialize(
        ...
        __ConverterBase_initialize(_token, _registry, _maxConversionFee);
    }
    ...
    function targetAmountAndFee(
        ...
        if (
            _targetToken == IDSToken(address(anchor)) &&
            reserves[_sourceToken].isSet
        ) return purchaseTargetAmount(_amount);
        if (
            _sourceToken == IDSToken(address(anchor)) &&
        ...
        returns (uint256, uint256)
    {
        ...
        if (totalSupply == 0)
            return (
                _amount.mul(PPM_RESOLUTION).div(reserves[reserveToken].weight),
                0
            );
    ...
    function buy(
        ...
        if (reserveToken == ETH_RESERVE_ADDRESS)
            ...
            require(
                msg.value == 0 &&
                    reserveToken.balanceOf(address(this)).sub(
                        reserveBalance(reserveToken)
                    ) >=
                    _amount,
                "ERR_INVALID_AMOUNT"
    ...
    function sell(
        ...
        // ensure that the input amount was already deposited
        require(
            _amount <= IDSToken(address(anchor)).balanceOf(address(this)),
            "ERR_INVALID_AMOUNT"
        );
        ...
        assert(
            amount < rsvBalance ||
                (amount == rsvBalance && _amount == tokenSupply)
        );
