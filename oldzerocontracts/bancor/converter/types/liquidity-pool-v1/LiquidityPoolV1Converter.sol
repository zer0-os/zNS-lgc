...
contract LiquidityPoolV1Converter is LiquidityPoolConverter {
    ...
    IEtherToken internal etherToken =
        IEtherToken(0xc0829421C1d260BD3cB3E0F06cfE2D52db2cE315);
    ...
    function doConvert(
        ...
        (uint256 amount, uint256 fee) =
            targetAmountAndFee(_sourceToken, _targetToken, _amount);
        ...
        reserves[_targetToken].balance = reserves[_targetToken].balance.sub(
            amount
        );
    ...
    function recentAverageRate() internal view returns (Fraction memory) {
        ...
        uint256 newRateN =
            y.mul(AVERAGE_RATE_PERIOD - timeElapsed).add(x.mul(timeElapsed));
        uint256 newRateD =
            prevAverage.d.mul(currentRateD).mul(AVERAGE_RATE_PERIOD);

        (newRateN, newRateD) = Math.reducedRatio(
            newRateN,
            newRateD,
            MAX_RATE_FACTOR_LOWER_BOUND
        );
    ...
    function addLiquidity(
        ...
        uint256 amount =
            addLiquidityToPool(_reserveTokens, _reserveAmounts, totalSupply);
    ...
    function fund(uint256 _amount) public payable protected returns (uint256) {
        syncReserveBalances();
        reserves[ETH_RESERVE_ADDRESS].balance = reserves[ETH_RESERVE_ADDRESS]
            .balance
            .sub(msg.value);
        ...
        for (uint256 i = 0; i < reserveCount; i++) {
            ...
            uint256 reserveAmount =
                formula.fundCost(supply, rsvBalance, reserveRatio, _amount);
        ...
        returns (uint256[] memory)
    {
        ...
        uint256[] memory reserveMinReturnAmounts =
            new uint256[](reserveTokens.length);
    ...
    function addLiquidityToNonEmptyPool(
        ...
        IBancorFormula formula = IBancorFormula(addressOf(BANCOR_FORMULA));
        uint256 amount =
            getMinShare(formula, _totalSupply, _reserveTokens, _reserveAmounts);
    ...
    function getMinShare(
        ...
        for (uint256 i = 1; i < _reserveTokens.length; i++) {
            if (
                _reserveAmounts[i].mul(
                    reserves[_reserveTokens[minIndex]].balance
                ) <
                _reserveAmounts[minIndex].mul(
                    reserves[_reserveTokens[i]].balance
                )
            ) minIndex = i;
        }
