...
contract LiquidityPoolV2Converter is LiquidityPoolConverter {
    ...
    function _validPoolToken(IDSToken _address) internal view {
        require(
            address(poolTokensToReserves[_address]) != address(0),
            "ERR_INVALID_POOL_TOKEN"
        );
    }
        ...
        validAddress(address(_secondaryReserveOracle))
    {
        ...
        // validate oracles
        IWhitelist oracleWhitelist =
            IWhitelist(addressOf(CHAINLINK_ORACLE_WHITELIST));
        require(
        ...
        else secondaryReserveToken = reserveTokens[0];
        ...
        LiquidityPoolV2ConverterCustomFactory customFactory =
            LiquidityPoolV2ConverterCustomFactory(
                address(
                    IConverterFactory(addressOf(CONVERTER_FACTORY))
                        .customFactories(converterType())
                )
            );
        ...
        uint256 primaryReserveStakedBalance =
            reserveStakedBalance(primaryReserveToken);
        ...
        if (primaryReserveStakedBalance == primaryReserveBalance) {
            if (
                primaryReserveStakedBalance > 0 || secondaryReserveBalance > 0
            ) {
                rebalance();
            }
        } else if (
            primaryReserveStakedBalance > 0 &&
            primaryReserveBalance > 0 &&
            secondaryReserveBalance > 0
        ) {
            rebalance();
        }
     ...
     */
    function setOracleDeviationFee(uint32 _oracleDeviationFee)
        public
        ownerOnly
    {
        require(
            _oracleDeviationFee <= PPM_RESOLUTION,
    ...
    function setReserveStakedBalance(
        ...
        uint256 _balance
    ) public ownerOnly only(CONVERTER_UPGRADER) validReserve(_reserveToken) {
     ...
     */
    function poolToken(IERC20Token _reserveToken)
        public
        view
        returns (IDSToken)
    {
     ...
     */
    function liquidationLimit(IDSToken _poolToken)
        public
        view
        returns (uint256)
    {
    ...
    function effectiveReserveWeights() public view returns (uint256, uint256) {
        Fraction memory rate = _effectiveTokensRate();
        (uint32 primaryReserveWeight, uint32 secondaryReserveWeight) =
            effectiveReserveWeights(rate);
        ...
        returns (uint256)
    {
        ...
        (uint256 amount, uint256 standardFee, uint256 totalFee) =
            targetAmountAndFees(
                _sourceToken,
                _targetToken,
                sourceTokenWeight,
        ...
        if (_sourceToken == ETH_RESERVE_ADDRESS)
            ...
            require(
                msg.value == 0 &&
                    _sourceToken.balanceOf(address(this)).sub(
        ...
        reserves[_targetToken].balance = reserveBalance(_targetToken).sub(
            amount
        );

        // update the target staked balance with the fee
        stakedBalances[_targetToken] = stakedBalances[_targetToken].add(
            standardFee
        );
        ...
        returns (uint256)
    {
        ...
        require(
            _reserveToken == ETH_RESERVE_ADDRESS
                ? msg.value == _amount
                : msg.value == 0,
        ...
        if (_reserveToken == ETH_RESERVE_ADDRESS) {
            reserves[ETH_RESERVE_ADDRESS].balance = reserves[
                ETH_RESERVE_ADDRESS
            ]
                .balance
                .sub(msg.value);
        ...
        if (maxStakedBalanceEnabled) {
            require(
                maxStakedBalances[_reserveToken] == 0 ||
                    ...
                    maxStakedBalances[_reserveToken],
        ...
        reserves[_reserveToken].balance = reserves[_reserveToken].balance.add(
            _amount
        );
        ...
        returns (uint256)
    {
        ...
        (uint256 reserveAmount, ) =
            removeLiquidityReturnAndFee(_poolToken, _amount);
        ...
        IPoolTokensContainer(address(anchor)).burn(
            _poolToken,
            msg.sender,
            _amount
        );
        ...
        // sync the reserve balance / staked balance
        reserves[reserveToken].balance = reserves[reserveToken].balance.sub(
            reserveAmount
        );
        uint256 newStakedBalance =
            stakedBalances[reserveToken].sub(reserveAmount);
     ...
     */
    function removeLiquidityReturnAndFee(IDSToken _poolToken, uint256 _amount)
        public
        view
        returns (uint256, uint256)
    {
        ...
        uint256 stakedBalance =
            stakedBalances[poolTokensToReserves[_poolToken]];

        if (_amount < totalSupply) {
            uint256 x =
                stakedBalances[primaryReserveToken].mul(AMPLIFICATION_FACTOR);
            uint256 y = amplifiedBalance(primaryReserveToken);
            (uint256 min, uint256 max) = x < y ? (x, y) : (y, x);
            uint256 amountBeforeFee =
                _amount.mul(stakedBalance).div(totalSupply);
        ...
        )
    {
        ...
        uint256 totalFee =
            targetAmount.mul(oracleDeviationFee).div(PPM_RESOLUTION).add(
                standardFee
            );
    ...
    function _effectiveTokensRate() private view returns (Fraction memory) {
        (uint256 latestRateN, uint256 latestRateD) =
    ...
    function rebalance() private {
        (
            reserves[primaryReserveToken].weight,
            reserves[secondaryReserveToken].weight
        ) = effectiveReserveWeights(externalRate);
    }
     ...
     * @return amplified balance
     */
    function amplifiedBalance(IERC20Token _reserveToken)
        internal
        view
        returns (uint256)
    {
        return
            stakedBalances[_reserveToken].mul(AMPLIFICATION_FACTOR - 1).add(
                reserves[_reserveToken].balance
            );
    }
     ...
     */
    function effectiveReserveWeights(Fraction memory _rate)
        private
        view
    ...
    function dispatchTokenRateUpdateEvent(
        ...
        emit TokenRateUpdate(
            _token1,
            _token2,
            token2Balance.mul(_token1Weight),
            token1Balance.mul(_token2Weight)
        );
    }
