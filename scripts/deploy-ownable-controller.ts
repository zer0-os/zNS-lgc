...
const logger = getLogger("scripts::deploy-ownable-controller");
...
async function main() {
  ...
  const controllerFactory = new OwnableController__factory(deploymentAccount);
  ...
  logger.log(`Deployed OwnableController to '${instance.address}'`);

  const deploymentRecord: DeployedContract = {
    name: "OwnableController",
  ...
  deploymentData.ownableController = deploymentRecord;
