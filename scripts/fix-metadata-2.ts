...
const loadUpdateFile = () => {
  ...
  toUpdate.domains = toUpdate.domains.map((e) => {
    const domainId = domainNameToId(`wilder.wheels.genesis.${e.id}`);
    return {
      id: domainId,
...
const main = async () => {
  ...
  const instance = RegistrarMDFix__factory.connect(registrarAddress, deployer);
