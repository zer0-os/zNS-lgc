...
describe("Domain", function () {
  ...
  it("registrar owner can create multiple domains and subdomains", async function () {
    await registrar.createDomain("foo", accs[0], accs[0], "someref0");
    const id0 = await registrar.getId(["foo"]);
    await registrar.createDomain("foo.bar", accs[0], accs[0], "someref1");
    const id1 = await registrar.getId(["foo", "bar"]);
    await registrar.createDomain("foo.baz", accs[0], accs[0], "someref2");
    ...
    await registrar.createDomain("community", accs[0], accs[0], "someref3");
    const id3 = await registrar.getId(["community"]);
    await registrar.createDomain(
      "community.dao",
      accs[0],
      accs[0],
      "someref4"
    ...
    const txr = await registrar.createDomain(
