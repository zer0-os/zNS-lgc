...
describe("Domain", function () {
  ...
  it("root can create multiple domains = fails at create limit", async function () {
    ...
    await registry.createDomain(bar, "bazchild", accs[0], zeroAddress);
  ...
  it("setting limitablePropertiesRule forbids changes after set", async function () {
    await registry.setLimitableProperties(foochild, "foobar");
    await registry.setLimitableProperties(foochild, "foobar1");
    await registry.setChildLimitablePropertiesRule(bar, 1);
    await registry.setChildLimitablePropertiesRule(foo, 1);
    await registry.setLimitableProperties(barchild, "barfoo");
    await expect(
      registry.setLimitableProperties(foochild, "foobar2")
    ).to.revertedWith("");
    await expect(
      registry.setLimitableProperties(barchild, "barfoo2")
    ).to.revertedWith("");
  });
});
